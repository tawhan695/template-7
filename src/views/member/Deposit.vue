                ...
                <div class="tab-content mb-5" id="myTabContent">
                  <div class="tab-pane fade show active" id="pro-tab-pane" role="tabpanel" aria-labelledby="pro-tab" tabindex="0">
                    ...
                    <div class="row mb-4 mt-3">
                      <div id="carosel_promo1" class="carousel slide">
                        <div class="carousel-indicators">
                          <button v-for="(promo, index) in  promotionList.data " :key="index" :class="index == 0 ? 'active' : ''" type="button"
                            data-bs-target="#carosel_promo1" :data-bs-slide-to="index" aria-current="true">{{ index + 1 }}</button>

                        </div>

                        <div class="carousel-inner">
                          <div v-for="(promo, index) in  promotionList.data " :key="index" class="carousel-item" :class="index == 0 ? 'active' : ''">

                            <div class="card m-3" v-bind:class="{ 'selected': selectPromotionId == promo.id }">
                              <a @click="selectPromotion(null)"><i class="fa fa-times"></i></a>
                              <div class="card-body p-0" @click="selectPromotion(promo)">
                                <img :src="promotionList.host + promo.imgdesc" alt="" class="img-fluid">
                                <p class="px-2">{{ promo.desc }}</p>
                              </div>
                            </div>
                          </div>
                        </div>
...
export default {
  ...
  computed: {
    promotionList() {
      let temp = this.option.getPromotions;
      temp.data = temp.data.filter(promo => (this.userStore.userProfile.role == 0 && promo.promotion_type == 'newmember') || (this.userStore.userProfile.role != 0 && promo.promotion_type == 'member') || (promo.promotion_type == 'allmember'))
      return temp;
    },
  },
  components: {
  ...
  mounted() {
    ...
    //Carousel
    const myCarousel = document.getElementById('carosel_promo1');
