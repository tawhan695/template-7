    ...
    <div class="h-100 w-100 pt-3">
      ...
      <div class="justify-content-center d-flex">
        <div class="container-fluid">
          ...
          <div class="row p-2">
            <div class="col-6 col-sm-3 col-lg-2 p-2" v-for="item in slot.gamesList" :key="item"
              v-show="item.active && search == ''">
              <!-- @click="showAlert_login" -->

              <div
                @click="item.productName === 'onesoft' || item.productName === 'onesoft2' ? gameOnesoft.gameloginonesoft(item.gameCode, item.productName,item.gameName) : games.gamelogin(item.gameLoginCode, item.productCode,item.gameName)">
                <img class="w-100 img-hover-zoom" style="border-radius: 15px"
                  :src="item.productName === 'onesoft' || item.productName === 'onesoft2' ? slot.host + item.imgUrl : ASKMEBET_URL + item.imgUrl" />
                <p class="text-white p-1 text-center" style="font-size: 0.7rem">
                  {{ item.gameName }}
                </p>
              </div>
            </div>
            <div class="col-6 col-sm-3 col-lg-2 p-2" v-for="item in datasearch" :key="item"
              v-show="item.active && search != ''">
              <!-- @click="showAlert_login" -->
...
export default {
  ...
  data() {
    return {
      slot: this.games.SLOT.find((e) => e.productCode === this.$route.params.name),
      search: "",
      searchSlot: [],
    };
  },
  ...
  computed: {
    datasearch() {
      ...
      if (this.search.length > 0) {
        for (const [key, value] of Object.entries(this.slot.gamesList)) {
