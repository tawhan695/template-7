...
export const parseTimestamp = (timestamp, format = '') => {
	...
	const date = timestamp.seconds
		...
		: new Date(timestamp); 
	 	try {
			if (format === 'HH:mm') {
				// console.log('HH:mm',date.getHours());
			...
			// console.log('format error' ,format,date,timestamp);
			return timestamp
		}
...
export const formatTimestamp = (date, timestamp) => {
	const timestampFormat = isSameDay(date, new Date()) ? 'HH:mm' : 'DD MMMM, HH:mm'
	const result = parseTimestamp(timestamp, timestampFormat)
	return timestampFormat === 'HH:mm' ? `วันนี้, ${result}` : result 
}
